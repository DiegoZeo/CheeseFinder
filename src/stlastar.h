...
public: // methods
	...
	unsigned int SearchStep(GameMap* map)
	{
...
public:
	...
	virtual bool GetSuccessors( AStarSearch<T> *astarsearch, T *parent_node, GameMap* map) = 0; // Retrieves all successors to this node and adds them via astarsearch.addSuccessor()
	virtual float GetCost( T &successor, GameMap* map ) = 0; // Computes the cost of travelling from this node to the successor node
